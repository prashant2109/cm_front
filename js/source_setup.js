...
app.directive('tasSource', function(){
    return {
            template :`<div class="source_div_cvr" ng-class="{'show_cvr_div':enable_source_flg}">
	 	<button class="btn btn-xs btn-info source_icn" ng-class="{'icn_css':enable_source_flg}"title="Upload PDF / HTML" ng-click="enable_source_data();" id="" ><i class="fa fa-cog" aria-hidden="true"></i></button>
	...
	<style>
		...
		.source_div_cvr #document_source{
			float:left;
			width: 100%;
			margin-top: 10px;
			padding: 5px 10px;
		}
		.source_div_cvr #document_source form input{
			background:#f9f9f9;
			font-size: 13px;
    			height: auto;
		}
		.source_div_cvr #document_source form input:focus{
			background: #f9f9f9;
			box-shadow: none;
		}
		.source_div_cvr .source_icn{
			height: 27px;
			margin: 2px;
			padding: 2px 8px;
			width: 30px;
		...
		.source_div ul.tabs li.current{ 
		    	background: #95b4ce !important;
		    	color: #fff;
			font-weight: 500;
		}
		.source_div .form-check-inline{
			display:block;
		}
		.meta_data_div .meta_data_input{
		...
		.source_div_cvr .form-check-label{
		  width:25%;
		  padding-bottom:5px;
		}
		.source_div_cvr label{
		  width:100%;
		}
		.download-button {
			background: #dae8f1 !important;

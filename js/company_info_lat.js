...
app.controller("company_info", function($scope,  $rootScope, $http, $timeout, $location, $filter, $sce, tasAlert, tasService){
	...
	/***********************************/
         $scope.load_state_drop = function(ress, eve){
                ...
                $timeout(function(){
                     console.log("hhhhhhhhhhhh", $scope.company_address)
	...
	$scope.cl_bk_default_settings	= function(res){
           if(res["message"] == 'done'){
                        if ('client_info' in res){
                          ...
                          /* $scope.client_data_dicts['vals'].push({'k':2, 'n':'data1'})
                           $scope.client_data_dicts['vals'].push({'k':3, 'n':'data3'})
                           $scope.client_data_dicts['vals'].push({'k':4, 'n':'data4'})
                           $scope.client_data_dicts['vals'].push({'k':5, 'n':'data5'})
                           $scope.client_data_dicts['vals'].push({'k':6, 'n':'data6'})
                           $scope.client_data_dicts['vals'].push({'k':7, 'n':'data7'})
                           $scope.client_data_dicts['vals'].push({'k':8, 'n':'data8'})*/
                        }
			$scope.basic_info_disp_dict.forEach(function(r){
				if(r.def_k in  res){
	...
	/***********************************/
         ...
         $scope.module_save_func = function(){
					  ...
					  $scope.url_data.forEach(function(r){
					      r['n'] = encodeURIComponent(r.n)
                            ...
                            $scope.company_address.forEach(function(r){
                                  console.log("gggggggggggggg", r)
                                /*  address: {k: "address", n: "Address", v: "q1w"}
                                  country: {k: "country", n: "Country"  v: {…}}
                                  fax: {k: "fax", n: "Fax", v: "111"}
                                  phone: {k: "phone", n: "Phone Number", v: Array(0), val: Array(2)}
                                  pincode: {k: "pincode", n: "Pincode", v: "qsqs"}
                                  state: {k: "state", n: "State", v: {…}}
                                  type: {k: "type", n: "Type", v: {…}}*/
                                  sdicts_v ={}
                                  sdicts_v['address'] = {'k': r['address']['k'], 'v':r['address']['v']}
                                  sdicts_v['country'] = {'k': r['country']['k'], 'v':r['country']['v']}
                                  sdicts_v['fax'] = {'k': r['fax']['k'], 'v':r['fax']['v']}
                                  sdicts_v['phone'] = {'k': r['phone']['k'], 'val':r['phone']['val']}
                                  sdicts_v['pincode'] = {'k': r['pincode']['k'], 'v':r['pincode']['v']}
            ...
            console.log('json datta::::::::::::::', post_data)
		  ...
		  $timeout(function(){$scope.$apply()});
                ...
                $scope.basic_info_disp_dict.forEach(function(res){
                       if (res.drp_lst && res.k != 'filing_frequency' && res.k != 'reporting_start_period'){
                          console.log("y:::::::::::::", res.drp_lst)
                          console.log("kkkkk:::::::::::::", res)
                          console.log("kkkkk:::::::::::::", $scope.cmp_info['company_mgmt'][res.k])
   ...
   // }
	...
	/***********************************/
         ...
         $scope.filter_client_names = function(){
                ...
                console.log('selected vals::::::::::::::', $scope.slct_vals)
         }
...
app.directive('compInfo', function(){
			...
			<div class="mod_inf">
			    ...
			    <div class="ts_box_menu app-sidebar" ng-class="{'active':side_bar_status }">
                                        <div  style="height:60px; padding: 2px 15px;">
							  ...
							  <div class="header-icon waves-effect dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="padding: 0px 5px;color: #fff; border: 1px solid #dfdfdf;">
							      <div class="header_icon_txt" title="{{data_vals[1]['n']}}" style="color:#black!important;">
                                                                <div style="float:left; width:60px;margin-top: 7px;margin-left: 10px;background-color: #e8e8e8 !important;border-radius: 2px;line-height: 25px;display: inline-block;" ng-repeat = "data in client_data_dicts['sel_res'] track by $index" title="{{data['n']}}">
                                                                   <div class="pop-ver-sel-cls" style="float: left;line-height: 25px;margin-left: 1px;width: 45px;    white-space: nowrap;overflow: ellipsis;overflow: hidden; cursor:auto" > 
								             <b>{{data['n']}}</b>
							           </div>
					...
					<div class="app-sidebar__inner" style= "height: calc(100% - 60px);">
...
<style>
		       .module-info {width: 100%;height: calc(100% - 30px);float:left;}
		       ...
		       .mod_inf .drop_down_class{height:40px; float:left; margin-left:2px; color: #012b3b;}
                      ...
                      /*********************************************Drop Down ******************************************************/
                       ...
                       //.mod_inf .displat-selected-version {width: 120px;height: 25px;margin-top: 7px;margin-left: 10px; background-color: #e6e6e6 !important; border-radius: 2px;line-height: 25px;display: inline-block;}
                     ...
                     //  .mod_inf .header_col{ width:98%; height:30px; background-color: #dae8f1;color:#3f6ad8; margin-top:5px;}
                       ...
                       .mod_inf .pop-ver-sel-cls-left{float: left; cursor:pointer;width: calc(100% - 20px);padding-left: 8px;white-space: nowrap; -ms-text-overflow: ellipsis; -o-text-overflow: ellipsis; text-overflow: ellipsis;overflow: hidden;}
                       .mod_inf .pop-ver-del-cls-right{float: right; cursor:pointer; width: 20px; text-align: center; color: #F44336; font-size: 15px;}
                       .mod_inf .dropdown-item-tas.active{background: #c7dcf0;}

                   /*    .mod_inf .setup-forms {min-height: 40px;}
                       .mod_inf .setup-forms .setup-desc {height: 39px;border-bottom: 1px solid #ddd;width: 100%;line-height: 39px;color: #888;cursor: pointer;}
                   */

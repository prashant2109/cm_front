...
app.controller("MonitorCntrl", function($scope, $rootScope, $http, $timeout, $location, $filter, $sce, tasAlert, tasService, uiGridConstants, uiGridTreeBaseService){
	...
	$scope.schedule_scope = {parent_scope: $scope};
        $scope.information_scope = { 
                         parent_scope: $scope,
                         'host': 'http://172.16.20.220:2061/tree_data?input=',
                         'cmd_ids': {split_cmd : 7024 , list_cmd : 199 , texo_1_cmd : 7025 , texo_grid_cmd : 7027, grid_type_cmd : 7031, cgi_cmd :22},
                         'tabs': $scope.all_tabs
                         };
	...
	/***********************************/
        $scope.sel_types = {"all_tables":["Tables"],"sen_table_desc": ["Sentences-Table Descriptions"], "sen_addit":["Sentences Additional"],"other_txt_t_tble": ['Entity', 'Key Value'], "foot_notes":["Foot-Notes"], 'all_tabs':["Tables", "Sentences-Table Descriptions", "Sentences Additional", 'Entity', 'Key Value', "Foot-Notes"], 'table_analysis_tabs': ['Tables', 'Foot-Notes'], 'text_to_table_tabs': ['Sentences Additional','Sentences-Table Descriptions' , 'Entity', 'Key Value']}
	/***********************************/
	$scope.refOptions = {
		id: 'ref_frame_id',
		htmlRef: true,
	...
	$scope.cb_get_slct_indus_func = function(res, flg){
                $scope.ps = false;
                if(res['message']){
                        if(res['message'] == 'done'){
	...
	function gridOptionsDoccolumnDefs_func(){
                ...
                },
		{
		    ...
		    cellEditableCondition:false,
				headerCellClass: 'hdr_cntr',
		    cellTemplate:
				`<div class="ui-grid-cell-contents row_col_grid_cell review slct_grd_arw" ng-click="grid.appScope.review_prjct_click_func(row.entity)" style="text-align: center;padding: 0px;">
	...
	$scope.init_func = function(){
		if(!(Object.keys($scope.side_menu_slcted_flg).length))
			$scope.side_menu_slcted_flg = $scope.side_menu_list[0];
		else{
			//$scope.side_menu_click_func($scope.side_menu_slcted_flg);
	...
	$scope.side_menu_click_func = function(menu, mmenu_k=menu['k']){
		...
		if($scope.side_menu_slcted_flg['k'] == 'crawling_monitoring'){
			...
			if($scope.side_menu_slcted_flg['k'] == 'slt'){
				...
				var dic = {'name': $scope.slcted_indus_dic['project_name']};
				dic = JSON.stringify(dic);
                                $scope.pre_taxo_path = 'http://172.16.20.220/DataModel/index.html?input='+dic;
				...
				var dic  = [{'project_id': $scope.slcted_indus_dic['project_id'], 'company_name': cmp_map[$scope.slcted_comp_dic['company_name']]||$scope.slcted_comp_dic['company_name'], 'doc_id': '14'}];
                                dic = JSON.stringify(dic);
				$scope.pre_taxo_path = 'http://172.16.20.220:2061/searchv_demo?input='+dic;
				...
				var doc_lst = [];
				for (var i = 0; i < $scope.slcted_comp_dic['info'].length; i++) {
				    var tid = $scope.slcted_comp_dic['info'][i];
					 doc_lst.push(tid['d']);
				}
				$scope.pre_taxo_path = 'http://172.16.20.220:2063/projectv_demo?view=Review&nop=1&cid='+$scope.slcted_comp_dic['company_id']+'&pid='+$scope.slcted_indus_dic['project_id']+'&doc_ids='+JSON.stringify(doc_lst);
				...
				if($scope.slcted_indus_dic['project_id'] == "FE"){
					...
					if($scope.slcted_comp_dic['company_id'] == 'Amazon'){
						$scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=FD&ProjectID=15&user_id=nitin&pid='+$scope.slcted_indus_dic['project_id'];
					}else if($scope.slcted_comp_dic['company_id'] == 'JPMorgan'){
						$scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=FD&ProjectID=16&user_id=nitin&pid='+$scope.slcted_indus_dic['project_id'];
					}else if($scope.slcted_comp_dic['company_id'] == 'AmericanAirlines'){
						$scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=FD&ProjectID=17&user_id=nitin&pid='+$scope.slcted_indus_dic['project_id'];
					}else if($scope.slcted_comp_dic['company_id'] == 'GAPInc'){
						$scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=FD&ProjectID=18&user_id=nitin&pid='+$scope.slcted_indus_dic['project_id'];
					}
				}else if( $scope.slcted_indus_dic['project_id'] == '40'){
					if($scope.slcted_comp_dic['company_id'] == 'HSBC-FixedIncome'){
						$scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=FD&ProjectID=21&user_id=nitin&pid='+$scope.slcted_indus_dic['project_id'];
					}else if($scope.slcted_comp_dic['company_id'] == 'HSBC-Liquidity'){
						$scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=FD&ProjectID=22&user_id=nitin&pid='+$scope.slcted_indus_dic['project_id'];
					}else if($scope.slcted_comp_dic['company_id'] == 'HSBC-Equity'){
						$scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=FD&ProjectID=23&user_id=nitin&pid='+$scope.slcted_indus_dic['project_id'];
					}else if($scope.slcted_comp_dic['company_id'] == 'HSBC-MultiAsset'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=FD&ProjectID=24&user_id=nitin&pid='+$scope.slcted_indus_dic['project_id']
                                        }else if($scope.slcted_comp_dic['company_id'] == 'UBS-Equity'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=FD&ProjectID=25&user_id=nitin&pid='+$scope.slcted_indus_dic['project_id']
                                        }else if($scope.slcted_comp_dic['company_id'] == 'WellsFargo-Equity'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=FD&ProjectID=26&user_id=nitin&pid='+$scope.slcted_indus_dic['project_id']
                                        }else if($scope.slcted_comp_dic['company_id'] == 'StateStreet-Equity'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=FD&ProjectID=27&user_id=nitin&pid='+$scope.slcted_indus_dic['project_id']
					}else if($scope.slcted_comp_dic['company_id'] == 'BlackRock-FixedIncome'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=FD&ProjectID=28&user_id=nitin&pid='+$scope.slcted_indus_dic['project_id']
					}
				}
			}else if($scope.side_menu_slcted_flg['k'] == 'exha_data'){
				return;
				...
				if($scope.slcted_indus_dic['project_id'] == 'FE'){
				 	$scope.pre_taxo_path = 'http://172.16.20.220:2062/projectv_demo?view=RCA&cid='+[$scope.slcted_comp_dic['company_id'], $scope.slcted_comp_dic['model_id'], $scope.slcted_comp_dic['info'].slice(-1)[0]['d']].join('_')+'&pid='+$scope.slcted_comp_dic['rc_id']+'&user='+$scope.slcted_comp_dic['rc_user'];
				}
			}else if($scope.side_menu_slcted_flg['k'] == 'edit_op_temp'){
				if( $scope.slcted_indus_dic['project_id'] == '39'){
                                        if($scope.slcted_comp_dic['company_id'] == 'Amazon'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=Modal&ProjectID=15&user_id=nitin';
                                        }else if($scope.slcted_comp_dic['company_id'] == 'JPMorgan'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=Modal&ProjectID=16&user_id=nitin';
                                        }else if($scope.slcted_comp_dic['company_id'] == 'AmericanAirlines'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=Modal&ProjectID=17&user_id=nitin';
                                        }else if($scope.slcted_comp_dic['company_id'] == 'GAPInc'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=Modal&ProjectID=18&user_id=nitin';
                                        }
                                }else if( $scope.slcted_indus_dic['project_id'] == '40'){
					if($scope.slcted_comp_dic['company_id'] == 'HSBC-Liquidity'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=Modal&ProjectID=22&user_id=nitin';
                                        }else if($scope.slcted_comp_dic['company_id'] == 'HSBC-FixedIncome'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=Modal&ProjectID=20&user_id=nitin';
                                        }else if($scope.slcted_comp_dic['company_id'] == 'BlackRock-FixedIncome'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=Modal&ProjectID=28&user_id=nitin';
                                        }else if($scope.slcted_comp_dic['company_id'] == 'HSBC-MultiAsset'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=Modal&ProjectID=24&user_id=nitin';
                                        }
					else if($scope.slcted_comp_dic['company_id'] == 'UBS-Equity'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=Modal&ProjectID=25&user_id=nitin';
                                        }else if($scope.slcted_comp_dic['company_id'] == 'WellsFargo-Equity'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=Modal&ProjectID=26&user_id=nitin';
                                        }else if($scope.slcted_comp_dic['company_id'] == 'StateStreet-Equity'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=Modal&ProjectID=27&user_id=nitin';
                                        }else if($scope.slcted_comp_dic['company_id'] == 'HSBC-Equity'){
                                                $scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=Modal&ProjectID=23&user_id=nitin';
                                        }
	
                                }else{
                                	$scope.pre_taxo_path = 'http://172.16.20.220:2061/projectv_demo?view=Modal&ProjectID=9&user_id=nitin';
				}
			}
                              ...
                              if ($scope.selected_company_data == '' || $scope.comp_sel_fun == true){
                                 ...
                                 temp_cmp_id = [$scope.slcted_comp_dic['company_id'], $scope.slcted_comp_dic['model_id'],  $scope.slcted_comp_dic['info'][0]['d']].join('_')
                              } 
                              else{var temp_cmp_id = $scope.selected_company_data}
	...
	$scope.output_sclted_sub_click_func = function(ky){
		...
		if($scope.slcted_indus_dic['project_id'] == "FE"){
			if($scope.output_sclted_sub_menu == 'op_view'){
				...
				dic = JSON.stringify(dic);
                                $scope.pre_taxo_path = 'http://172.16.20.229:9888/validation_demo?input='+dic;
			}	
		}
		$timeout(function(){
			var ifrm_dom = document.getElementById('slt_frame_id');
	...
	$scope.review_prjct_click_func = function(row){
		...
		if($scope.side_menu_slcted_flg['k'] == 'toc'){
			...
			if($scope.slcted_indus_dic['project_id'] == "60"){
				...
				dic = JSON.stringify(dic);
				$scope.pre_taxo_path = 'http://172.16.20.220:2058/Analytics_Demo?input='+dic;
			}
		}else if($scope.side_menu_slcted_flg['k'] == 'exha_data'){
                       $scope.comp_sel_fun = false
                       $scope.information_scope.tabs = $scope.sel_types.all_tabs
                        var temp_cmp_id = $scope.slcted_comp_dic['company_name']+"_"+$scope.slcted_indus_dic['project_id']+"_"+row['d']
               ...
               else if($scope.side_menu_slcted_flg['k'] == 'table_analys'){
                        $scope.comp_sel_fun = false
                        var temp_cmp_id = $scope.slcted_comp_dic['company_name']+"_"+$scope.slcted_indus_dic['project_id']+"_"+row['d']
                        temp_cmp_id = [$scope.slcted_comp_dic['company_id'], $scope.slcted_comp_dic['model_id'], row['d']].join('_')
                        $scope.selected_company_data = temp_cmp_id
                        //{"all_tables":"Tables","sen_table_desc": "Sentences-Table Descriptions", "sen_addit":"Sentences Additional","other_txt_t_tble": ['Entity', 'Key Value'], "foot_notes":"Foot-Notes"}
                        $scope.information_scope.tabs = $scope.sel_types.table_analysis_tabs
                        $timeout(function(){
                        $scope.information_scope.scope.move_direct(temp_cmp_id)

                        }, 200); }
                else if($scope.side_menu_slcted_flg['k'] == 'text_to_table'){
                        $scope.comp_sel_fun = false
                        var temp_cmp_id = $scope.slcted_comp_dic['company_name']+"_"+$scope.slcted_indus_dic['project_id']+"_"+row['d']
                        temp_cmp_id = [$scope.slcted_comp_dic['company_id'], $scope.slcted_comp_dic['model_id'], row['d']].join('_')
                        $scope.selected_company_data = temp_cmp_id
                        $scope.information_scope.tabs = $scope.sel_types.text_to_table_tabs
                        $timeout(function(){
                        $scope.information_scope.scope.move_direct(temp_cmp_id)
	...
	/***********************************/
		...
		if($scope.slcted_indus_dic['project_id'] == "FE"){
			...
			if($scope.side_menu_slcted_flg['k'] == 'data_builder_rca'){
			 	$scope.pre_taxo_path = 'http://172.16.20.220:2063/projectv_demo?view=Review&nop=1'+'&cid='+$scope.slcted_comp_dic['company_id']+'&pid='+$scope.slcted_indus_dic['project_id'];//+'&data_type=1&doc_id='+row['d']
			}else 
			 	$scope.pre_taxo_path = 'http://172.16.20.220:2058/Analytics_Demo?input='+dic;
			$timeout(function(){
        	                var ifrm_dom = document.getElementById('slt_frame_id');
		...
		$scope.$apply(function(){
			...
			var post_data = {"cmd_id": 4,"project_id": $scope.doc_updated_dic['project_id'], 'crid': $scope.doc_updated_dic['company_id'],"user_id":"tas"}
				...
				if(res['message']=='done'){
					...
					/*$scope.$apply(function(){
						$scope.gridOptionsDoc.data	= $scope.gridOptionsDoc.data
						$scope.industry_type_list	= $scope.industry_type_list
					});*/
				}else{
					tasAlert.show(res['message'], 'error', 1000);
				}

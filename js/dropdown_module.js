...
app.controller("Dropdown",function($http, $scope, $timeout, tasAlert, tasService){
    ...
    $scope.datas = $scope.config.data || [{'n':'Item1','k': '1'},{'n':'Item2','k': '2'},{'n':'Item 3','k': '3'}];
    /******************************************/
    $scope.show = false;
    ...
    $scope.dataprenext = function(opt){
        ...
        if(!$scope.selected){
             if($scope.datas.length){
                  $scope.change($scope.datas[0]);
                  kyblk = false;
             }
        }
            
        var index = $scope.datas.findIndex($scope.arrindexfind,$scope.selected);
    ...
    return {
        ...
        template:`<div class="dropModule">
            <div class="drop_header">
                <div class="drop_preview" ng-click="dataprenext('preview')" ng-if="config.options['arrow'] != false" ng-class="{'clk_disble': datas[0]['k'] == selected['k'],'brdl':config.options['border-left'] != false,'brdt':config.options['border-top'] != false,'brdb':config.options['border-bottom'] != false}"><i class="fa fa-chevron-left"></i></div>
                ...
                <div class="drop_next" ng-click="dataprenext('next')" ng-if="config.options['arrow'] != false" ng-class="{'clk_disble': datas[datas.length - 1]['k'] == selected['k'],'brdr':config.options['border-right'] != false,'brdt':config.options['border-top'] != false,'brdb':config.options['border-bottom'] != false}"><i class="fa fa-chevron-right"></i></div>
            </div>
            <div class="drop_body" ng-if="show">
                ...
                <ul class="drop_all" ng-class="{'wtout_fltr': config.options['filter'] == false}">
                    <li class="drop_items" ng-repeat="list in (filterItems['data'] = ( datas | filter:item_filter)) track by $index" ng-click="list['disable'] != true && change(list)" ng-class="{'active': list['k'] == selected['k'],'move':list['k'] == keyupSelection['k'],'clk_disble': list['disable'] == true}" title="{{list['n']}}" ng-bind-html="list['n']"></li>
                </ul>
            </div>	
        </div>
        <style>
        ...
        .dropModule .drop_all .drop_items{line-height: normal;padding: 7px 10px;list-style: none;cursor:pointer;border-bottom: 1px solid #ebebeb;display: block;}

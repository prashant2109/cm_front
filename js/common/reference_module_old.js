...
app.controller('RefController', function($scope, $rootScope, $http, $timeout, $location, $filter, $sce, refIframe, tasAlert){
    ...
    $scope.show_pagination = function(pa){
        $scope.ref_pages    = Object.keys(pa)
        $scope.ref_pages.sort(function(a, b){return a-b})
        $scope.p_st         = 0
        $scope.p_end        = 10
        $scope.high_list = pa; 
        //console.log('high_list', $scope.high_list);
    }
    $scope.page_prev    = function(){
        if($scope.p_st > 0){
            var p_st    = $scope.p_st
            var remaning    = $scope.ref_pages.slice(0, $scope.p_st)
            if (remaning.length > 10)
                $scope.p_st     = $scope.p_st - 10
            else 
                $scope.p_st     = $scope.p_st - remaning.length
            $scope.p_end    = p_st
        }
    }
    $scope.page_next    = function(){
        if($scope.p_end < $scope.ref_pages.length){
            $scope.p_st = $scope.p_end
            var remaning    = $scope.ref_pages.slice($scope.p_st, $scope.p_st+10)
            if(remaning.length > 10)
                $scope.p_end = $scope.p_end+10
            else
                $scope.p_end = $scope.p_end+remaning.length
        }
        //console.log($scope.p_st, $scope.p_end, $scope.ref_pages.length)
    }
    $scope.individual_highlight = function(ke,val,iframe_id){
        $scope.pg_active = ke;
        
        var iframe_dom = document.querySelector('#'+iframe_id);
        var frame_window = iframe_dom.contentWindow;
        //var ref_divs = frame_window.document.querySelectorAll('id_'+ke); 
        var ref_divs = frame_window.document.getElementById('id_'+ke);     

        var x  = $(ref_divs).position();        
        var left = x.left;
        var tp =  x.top;
        
        frame_window.scrollTo(left-150, tp - 150);
        /*if(ref_divs.length){
            var left = ref_divs[0].getBoundingClientRect().left
            var tp = ref_divs[0].getBoundingClientRect().top
            frame_window.scrollTo(left-150, tp - 150);
        }*/

    }

    $scope.close_bottom_div = function(){
        $scope.config.parent_scope.close_bottom_div();
    }
	...
	$scope.clear_table_highlight = function(id){
            refIframe.clearHighlights_dbbox(id);
        }
    	/***********************************/
	$scope.pno_filter_inp_func = function(data, txt=''){
    	...
    	/***********************************/
	$scope.reload_frame = function(config){
		...
		if (config.page_change){
			...
			$scope.reload_frame(config);
		}
		if(config['active'] == 'pdf'){
		    var bbox = [];
		    var coord = [];
		    if(Object.keys(config['ref']).length){
			bbox = config['ref']['bbox'] || [];
			coord =  config['ref']['coord'] || [];
			bg = config['ref']['bg'] || '';
			border = config['ref']['border'] || '';
			class_name = config['ref']['c_name'] || 'parent_iframe_tag';
			clr_flg = true;
			if('clr_flg' in config['ref'])
				clr_flg = config['ref']['clr_flg'];
		    }
		    refIframe.pdfHighlight(config, bbox, coord, class_name, clr_flg, bg, border);
		}else{
		    var xml_list = [];
		    if('x' in (config['ref'] || {}))
			xml_list = config['ref']['x'].split('$');
		    config['ref']['xml_list'] = xml_list;
		    refIframe.tableHighlight(config, 'parent_iframe_tag', true, '', '');  
		}
    	}
    	/***********************************/
	$scope.page_no_change_func = function(config, pos){
		if(pos == 'prev'){
		    rw = config.pno_list[config.selected_pno - 2];
		}else if(pos == 'next'){
		    rw = config.pno_list[config.selected_pno];
		}
	...
	/***********************************/
    	window.reset_txo_pop_func = function(){
		$scope.txo_list_pop_flg = {'k': false};
	...
	/***********************************/
		...
		if($scope.txo_list_pop_f_lft_t_id['dropdown'].length){
		    if(flg){
			tasAlert.show('Please select from sidedropdown.', 'warning', 1000);
			return;
		    } 
		}
		...
		if(Object.keys(slcted_dic).length && 'meta_flg' in slcted_dic && slcted_dic['meta_flg']=='Y'){
		    tasAlert.show('This Meta data don\'t have edit option.', 'warning', 2000);
		    return;
		}
		var dic = {sted: "", v: $scope.main_input_txtarea, x: "", 'ref': slct_obj['cids'], 'updated': "Y", 'fcd': slct_obj['from_custom_dom']};
	...
	window.show_txt_proper_slct_pop = function(){
		if($scope.config.dropDown){
                	$scope.txo_list_pop_slted_obj = {};
                	tasAlert.show('Please select proper text.', 'warning', 1000);
                	$scope.$apply();
		}
        }
	/***********************************/
	...
	$scope.html_ref_clear_func = function(){
		$scope.clear_pdf_highlight($scope.config.id);
    	}
	/***********************************/
		...
		template:`
				<div class="rmc">
					<div class="rmct">
                        ...
                        <div class="">                            
                            <ul class="pagination" ng-if="ref_pages.length">
                              <li title="Previous" ng-if="p_end > 10"><a ng-click="page_prev()"><i class="fa fa-angle-double-left" aria-hidden="true" title="Previous"></i></a></li>  
                              <li ng-repeat="k in ref_pages.slice(p_st, p_end)" ng-click="individual_highlight(k,high_list[k],'iframe1')" ng-class="{active:k==pg_active}"><a href="#" ng-class="{active:k==pg_active}">{{k}}</a></li>
                              <li title="Next" ng-if="p_end < ref_pages.length"><a ng-if="p_end < ref_pages.length" ng-click="page_next()"><i class="fa fa-angle-double-right" aria-hidden="true" title="Next"></i></a></li>  
                            </ul>
                        </div> 
						<div class="rmctr">
							  <div class="pull-left" ng-if="config['htmlRef'] && config['active']=='html' && config['htmlType'] == 'pdf'">
				                          	<div class="rmcticn waves-effect pull-left" title="Clear Selection" ng-click="html_ref_clear_func()"><i class="fa fa-eraser" aria-hidden="true"></i></div>
                          					<div class="rmcticn waves-effect pull-left" title="Multiselection" ng-click="html_ref_multi_func()" ng-class="{active: html_ref_multi_flg}"><i class="fa fa-object-ungroup" aria-hidden="true"></i></div>
								...
								<div class="rmcticn waves-effect pull-left" ng-click="page_no_change_func(config, 'next')" ng-class="{rmc_disable_btn: config.selected_pno == config.pno_list.length || config.pno_list.length==0}"><i class="fa fa-chevron-right" aria-hidden="true"></i></div>
							  </div>
					...
					<div class="rmcb">
						<iframe src="{{config['path']}}" id="{{config['id']}}"></iframe>
					</div>
				</div>
					...
					iframe{width: 100%;height: 100%;border: 0px;}
			                ...
			                .rmctlb_id{float:left;margin-left: 10px;line-height: 30px;color: #006f98;}
                			.rmctlb_id_s{font-weight: bold;}
					...
					.txo_list_pop_f_lft_c_sv {color: #fff;float: inherit;height: 30px;display: block;background: #36c851;padding: 0px 10px;line-height: 30px;font-size: 12px;cursor: pointer;}
					.txo_list_pop_f_lft_c_ipt {display: block;width: 260px;padding: 5px 6px;font-size: 13px;line-height: 1.42857143;color: #555;background-color: #fff;background-image: none;border: 1px solid #ccc;border-radius: 0px;float: left;margin-left: 8px;}
					.txo_list_pop_f_lft_c_del {float: right;display: block;text-align: center;font-size: 21px;cursor: pointer;width: 27px;height: 30px;background-color: #e0463b;background-size: 10px 10px;box-shadow: 0 6px 10px rgba(0, 0, 0, 0.2);color: #fff;line-height: 30px;}
...
app.factory('refIframe', function($timeout){
	...
	function highlightsbbox_withclass(id, bboxs, info, class_name, flag, bg, border){
        ...
        function createDiv(color, x, y, w, h,opc,bor,ifrm, elmid){
           var iframe_2 = document.getElementById(ifrm);
           var d = iframe_2.contentDocument.createElement('div');
           d.className = "div_bbox";
           d.setAttribute('div_bbox_id', elmid)
           d.setAttribute('id', 'id_'+elmid) 
           ...
           d.style.opacity = opc;
           d.style.border = bor;
           iframe_2.contentDocument.getElementsByTagName('body')[0].appendChild(d);
        }

        obj.clearHighlights_dbbox = function(iframe_id){
            var iframe_dom = document.querySelector('#'+iframe_id)
            var frame_window = iframe_dom.contentWindow;
        ...
        obj.clearTableHighlight = function(iframe_id){
                ...
                if(!iframe_dom)
                    return;
                var frame_window = iframe_dom.contentWindow;
                Array.prototype.forEach.call(frame_window.document.querySelectorAll('[role="tas"]'),function(node){
                ...
                obj.tableHighlight = function(config, class_name, clear_flag){
            console.log(config['ref'])
            var iframe_id = config['id'] || '';
            var xml_list = [];
            var char_list = [];
            if('xml_list' in config['ref']){
                xml_list = config['ref']['xml_list'] || [];
            }
        
            if('c' in config['ref']){
                char_list.push(config['ref']['c']);
            }
            var path = config['path'];
            if(iframe_id=='')
                return;
            ...
            if(doc_path != temp ){
                    ...
                    iframe_dom.setAttribute('src', temp);
                    iframe_dom.onload= function (){ setTimeout(function(){obj.tableHighlight(config, class_name, clear_flag)},200)}
                    //time = 1000;
            }
       
                console.log('IN', config['ref'])
                var frame_window = iframe_dom.contentWindow;
                if(clear_flag){
                ...
                xml_list.forEach(function(v,a){
                   if(config['html_type'] == 'pdf'){
                        //var dom = frame_window.document.querySelector('span[id='+v+']');
                        var dom = frame_window.document.querySelector('span[id="'+v+'"]');
                   }else{
                        var dom = frame_window.document.querySelector('[customindex="'+v+'"]');
                   }
                   console.log('first_child', dom, v); 
                   var range    = iframe_dom.contentWindow.document.createRange();
                   var sel      = window.getSelection();
                    var elm_len = dom.firstChild.length; 
                    char_arr = char_list
                    var char_val = char_arr[a].split('_');
                    if((((char_val[0] != '') && (char_val[1] != '')) && ((parseInt(char_val[0]) < parseInt(char_val[1])) && (parseInt(char_val[1]) <= dom.firstChild.length))) && (char_arr[a] != undefined)){
                        s   = char_val[0];
                        e   = char_val[1];
                    }else{
                        s   = 0;
                    ...
                    var elm = dom
                    range.setStart(elm.firstChild,s);
                    range.setEnd(elm.firstChild,e);
                    sel.removeAllRanges();
                    console.log(config['ref'], range.getBoundingClientRect)
                    if (range.getBoundingClientRect) {
                        var get_client_rect = range.getClientRects()
                        var get_keys_gcr    = Object.keys(get_client_rect);
                        for(rect_k in get_keys_gcr){
                            var rect            = get_client_rect[get_keys_gcr[rect_k]];
                            var x1              = rect.right - rect.left;
                            var y1              = rect.bottom - rect.top;
                            var x               = rect.left+frame_window.document.documentElement.scrollLeft;
                            var y               = rect.top+frame_window.document.documentElement.scrollTop;
                            createDiv('rgba(245, 209, 86, 0.25)', x, y, x1, y1,'1','1px solid #FFC500',config['id'], config['ref']['ID']);
                        }
                            
                    }
                });

                var ref_divs = frame_window.document.getElementsByClassName("div_bbox")
                if(ref_divs.length){
                    var left = ref_divs[0].getBoundingClientRect().left
                    var top = ref_divs[0].getBoundingClientRect().top
                    frame_window.scrollTo(left-150, top - 150);
                }

        }


        obj.tableHighlight_old = function(config, class_name, clear_flag){
		var iframe_id = config['id'] || '';
		var xml_list = config['ref']['xml_list'] || [];
		var path = config['path'];
		if(iframe_id=='')
			return;
                var iframe_dom = document.querySelector('#'+iframe_id)
                if(!iframe_dom)
                        return;
                var doc_path = iframe_dom.getAttribute('src') || '';
                //var temp = "src/iframe_track_v2.html?"+path
                var temp = path;
                var time = 0;
                if(doc_path != temp){
                        iframe_dom.setAttribute('src', temp);
                ...
                $timeout(function(){
            	    ...
            	    frame_window.document.documentElement.scrollTop = 0;
            	    frame_window.document.documentElement.scrollLeft = 0;
		    xml_list.forEach(function(v){
		       var dom = frame_window.document.querySelector('[customindex="'+v+'"]');
		       if(!dom){
			   var n_id   = v.split('#').filter(function(t){return (t != '');});
			   for(var a=0; a<n_id.length; a++){
				    var x_id_is =n_id[a];
				    dom = frame_window.document.querySelector('[customindex*="'+x_id_is+'"]');
				    if(!dom)
					continue
        ...
        obj.pdfHighlight = function(config, bbox, info, class_name, clear_flag, bg='', border=''){
		var iframe_id = config['id'];
		var path = config['path'];
                var iframe_dom = document.querySelector('#'+iframe_id)
                if(!iframe_dom)
            ...
            if(doc_path != temp){
                 ...
                 window.highlight_pdf = function(id){
                        $timeout(function(){
                                highlightsbbox_withclass(iframe_id, bbox, info, class_name, clear_flag, bg, border);
                        });
                 }
            }else{
                highlightsbbox_withclass(iframe_id, bbox, info, class_name, clear_flag, bg, border);
